...
Function Invoke-AzAutomationModuleImport {
    [CmdletBinding()]
    param (
    ...
    process {
        Write-verbose "Testing the avaliability of Automation Account '$AutomationAccountName'"
        $FindAccount = Get-AzAutomationAccount | Where-Object { $_.AutomationAccountName -eq "$AutomationAccountName" }
        if ([string]::IsNullOrWhiteSpace($FindAccount)) {
            Write-Error "Cannot find instance of the Automation Account '$AutomationAccountName'. Terminating!"
            Break
        }
        try {
            $ErrorActionPreference = 'Stop'

            Write-verbose "Getting Modules present in Automation Account '$AutomationAccountName'"
            ...
            foreach($mod in $AzModuleList)
            {
                ...
                Write-verbose "[$count/$(@($AzModuleList).count)] Importing module to Automation Account '$automationaccountName', contentlink = $($ImportSplat.ContentLinkUri)"
...
Invoke-AzAutomationModuleImport -AutomationAccountName "Philautomation" -Verbose -ModuleName azuread -WaitForCompletion